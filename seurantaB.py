...
def save_portfolios(user_id, portfolios):
    with sqlite3.connect(DB_FILE) as conn:
        ...
        # Poista kaikki vanhat salkut ja niihin liittyvät kohteet tälle käyttäjälle
        # Tämä varmistaa, että tietokanta pysyy siistinä eikä tuplaa dataa
        ...
        if old_portfolio_ids:
            cursor.execute(f"DELETE FROM assets WHERE portfolio_id IN ({','.join(['?']*len(old_portfolio_ids))})", old_portfolio_ids)
